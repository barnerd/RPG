...
func ability_performed(_actor: BattleActor, _ability: Ability, _targets: Array[BattleActor]) -> void:
	...
	var hitChance: float = CombatActivityReport.get_hit_chance(_actor.dex, _targets[0].dex)
	var criticalChance: float = CombatActivityReport.get_critical_chance(_actor.dex);
	var criticalStrength: float = CombatActivityReport.get_critical_strength(_actor.str);
